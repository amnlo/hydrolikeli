...
## subset.layout <- function(sudriv, keep.frac=0.3){
##     p <- rollmean(sudriv$input$inputobs[,2], k=5, fill=0)
##     p <- interp.precip(sudriv$input$inputobs[,1], p, sudriv$layout$layout[sudriv$layout$calib,])
##     keep <- p > 0.1
##     keep[sample(1:length(keep), size=round(length(keep)*keep.frac))] <- TRUE
##     sudriv$layout$calib <- sudriv$layout$calib[keep]
##     return(sudriv)
## }
...
## plot.dQdt <- function(dat, list.su, standardize=TRUE){
##     n.case <- length(unique(dat$case))
##     if(n.case > 1){warning("plot.dQdt not implemented for multiple models yet...")}
##     dQdt.mod <- calc.dQdt(c(list.su[[1]]$predicted$det),standardize=standardize)
##     n.samp <- nrow(list.su[[1]]$predicted$sample)
##     ## if(n.case > 1){
##     ##     for(i in 2:(n.case)){
##     ##         flash <- c(flash,c(apply(list.su[[i]]$predicted$sample,1,calc.flashiness)))
##     ##         n.samp <- c(n.samp,nrow(list.su[[i]]$predicted$sample))
##     ##     }
##     ## }
##     dQdt.obs <- calc.dQdt(subset(dat, case==case[1])$obs[(length(list.su[[1]]$layout$calib)+1):sum(dat$case==dat$case[1])], standardize=standardize)
##     print(cor(c(list.su[[1]]$predicted$det),subset(dat, case==case[1])$obs[(length(list.su[[1]]$layout$calib)+1):sum(dat$case==dat$case[1])]))
##     dat.dQdt <- data.frame(dQdt.obs=dQdt.obs, dQdt.mod=dQdt.mod, case=dat$case[1])
##     ## dat.dQdt <- subset(dat.dQdt, dQdt.obs<0)
##     g.obj <- ggplot(dat.dQdt, aes(x=dQdt.obs, y=dQdt.mod)) + geom_point() + labs(x="observed", y="modelled", caption=paste("R^2 = ",round(cor(dQdt.obs,dQdt.mod,use="complete.obs")^2,2))) + geom_abline(slope=1, intercept=0, col="red")+ theme_bw(base_size=24) ##+ theme(axis.text=element_text(size=24), axis.title=element_text(size=24))
##     plot(g.obj)
## }
...
transform.residuals <- function(su, Q.obs, Q.mod, P=NA, check.innovation=FALSE, t.obs=NA, rel.tol = .Machine$double.eps^0.25){
    ## Define function used for numerical integration -------------------------------------------------
    ## This is the product of the probability densitiy functions that defines the density of the innovation. This has to be integrated to judge the distributional assumptions on the level of innovation
    f.inno.t <- function(x, Q.obs.prev, Q.sim.prev, y, scale.prev, sc, de, dff, gmm=NA){
                e1 <- qnorm(pt((Q.obs.prev-Q.sim.prev)/scale.prev,df=dff),mean=0,sd=1)
                e2 <- qnorm(pt((x-y)/sc,df=dff),mean=0,sd=1)
                prob <- exp(dt((x-y)/sc,df=dff,log=TRUE) - log(sc) +
                    dnorm(e2,mean=e1*exp(-de),sd=sqrt(1-exp(-2*de)),log=TRUE) -
                    dnorm(e2,mean=0,sd=1,log=TRUE))
                return(prob)
    }
    f.inno.skt <- function(x, Q.obs.prev, Q.sim.prev, y, scale.prev, sc, de, dff, gmm){
                e1 <- qnorm(pskt((Q.obs.prev-Q.sim.prev)/scale.prev,df=dff,gamma=gmm),mean=0,sd=1)
                e2 <- qnorm(pskt((x-y)/sc,df=dff,gamma=gmm),mean=0,sd=1)
                prob <- exp(log(dskt((x-y)/sc,df=dff,gamma=gmm)) - log(sc) +
                    dnorm(e2,mean=e1*exp(-de),sd=sqrt(1-exp(-2*de)),log=TRUE) -
                    dnorm(e2,mean=0,sd=1,log=TRUE))
                prob[!is.finite(prob)] <- 0
                return(prob)
    }
    f.inno.n <- function(x, Q.obs.prev, Q.sim.prev, y, sd.prev, sdd, de){
                e1 <- qnorm(pnorm(Q.obs.prev,mean=Q.sim.prev,sd=sd.prev),mean=0,sd=1)
                e2 <- qnorm(pnorm(x,mean=y,sd=sdd),mean=0,sd=1)
                prob  <- exp(dnorm(x,mean=y,sd=sdd,log=TRUE) +
                    dnorm(e2,mean=e1*exp(-de),sd=sqrt(1-exp(-2*de)),log=TRUE) -
                    dnorm(e2,mean=0,sd=1,log=TRUE))
                return(prob)
    }
    mypt <- function(q, df, gamma=NA) pt(q=q, df=df)
    ## back-transform residuals to a standard normal distribution assuming they are t-distributed
    Q0 <- su$likelihood$parameters["C1Wv_Qstream_Q0_lik"]
    sd0<- su$likelihood$parameters["C1Wv_Qstream_sd0_lik"]
    a  <- su$likelihood$parameters["C1Wv_Qstream_a_lik"]
    b  <- su$likelihood$parameters["C1Wv_Qstream_b_lik"]
    c  <- su$likelihood$parameters["C1Wv_Qstream_c_lik"]
    d  <- su$likelihood$parameters["C1Wv_Qstream_d_lik"]
    tau<- su$likelihood$parameters["C1Wv_Qstream_tau_lik"]
    df <- exp(su$likelihood$parameters["C1Wv_Qstream_df_lik"]) + 2
    gamma <- su$likelihood$parameters["C1Wv_Qstream_gamma_lik"]
    n <- length(Q.obs)
    sd <- a*sd0*(Q.mod/Q0)^c + b
    if(any(is.na(t.obs)) & check.innovation){warning("t.obs contains missings"); return(NA)}
    lower <- -1*quantile(sd,0.8) # lower bound for numerical integration
    res.stdnorm <- rep(NA, length(Q.obs))
    res.stdnorm <- rep(NA, length(Q.obs))
    sd.t <- 1
    prob.t <- mypt
    if ( df < Inf ){
        if(grepl("skewt", su$settings$f.likeli)){
            prob.t <- pskt
            f.inno <- f.inno.skt
            myfun1 <- function(x,df){2*x*dt(x,df)}
            myfun2 <- function(x,df){2*x^2*dt(x,df)}
            m1 <- integrate(f=myfun1, lower=0, upper=Inf, df=df)$value
            m2 <- integrate(f=myfun2, lower=0, upper=Inf, df=df)$value
    ...
    scale <- sd/sd.t
    if(su$settings$f.likeli == "LogLikelihoodHydrology_noautocorr"){
        for(i in 1:length(Q.obs)){
            if(Q.obs[i]>0){
                res.stdnorm[i] <- qnorm(pt((Q.obs[i]-Q.mod[i])/scale[i],df=df))
            }else{## sample from the tail of the distribution that is below zero
                rndun <- runif(1)
                cumuzero <- pt(-1*Q.mod[i]/scale[i],df=df)
                res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
            }
        }
    }
    if(su$settings$f.likeli == "LogLikelihoodHydrology_wls"){
        for(i in 1:length(Q.obs)){
            if(Q.obs[i]>0){
                res.stdnorm[i] <- qnorm(pnorm(Q.obs[i], mean=Q.mod[i], sd=sd[i]))
            }else{## sample from the tail of the distribution that is below zero
                rndun <- runif(1)
                cumuzero <- pnorm(-1*Q.mod[i],sd=sd[i])
                res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
            }
        }
    }
    if(grepl("LogLikelihoodHydrology_la2", su$settings$f.likeli)){
        if (Q.obs[1] > 0){
            eta.2 <- qnorm(prob.t((Q.obs[1]-Q.mod[1])/scale[1], df=df, gamma=gamma),mean=0,sd=1)
        }else{
            rndun <- runif(1)
            cumuzero <- prob.t(-1*Q.mod[1]/scale[1],df=df,gamma=gamma)
            eta.2 <- qnorm(p=cumuzero*rndun)
        }
        res.stdnorm[1] <- eta.2
        for ( i in 2:n ){
            eta.1 <- eta.2
            if(check.innovation & (Q.mod[i]/Q.mod[i-1]/(t.obs[i]-t.obs[i-1])<d) & (Q.obs[i-1] > 0)){
                if(Q.obs[i]>0){
                    ##integr <- min(integrate(f=f.inno, lower=lower, upper=Q.obs[i], Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], scale.prev=scale[i-1], sc=scale[i], de=(t.obs[i]-t.obs[i-1])/tau, dff=df, gmm=gamma, subdivisions=2000)$value,1)
                    ##res.stdnorm[i] <- qnorm(integr)
                    delta <- (t.obs[i]-t.obs[i-1])/tau
                    eta.2 <- qnorm(prob.t((Q.obs[i]-Q.mod[i])/scale[i],df=df,gamma=gamma),mean=0,sd=1)
                    res.stdnorm[i] <- qnorm(pnorm(eta.2, mean=eta.1*exp(-1*delta), sd=sqrt(1-exp(-2*delta))))
                }else{
                ...
                if ( Q.obs[i] > 0 ){
                    res.stdnorm[i] <- qnorm(prob.t((Q.obs[i]-Q.mod[i])/scale[i],df=df,gamma=gamma))
                }else{
                    rndun <- runif(1)
                    cumuzero <- prob.t(-1*Q.mod[i]/scale[i],df=df,gamma=gamma)
                    res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
                }
            }
        }
    }else if(grepl("LogLikelihoodHydrology_la3", su$settings$f.likeli)){
        for(i in 1:2){
            if (Q.obs[i] > 0){
                eta.2 <- qnorm(prob.t((Q.obs[i]-Q.mod[i])/scale[i], df=df, gamma=gamma),mean=0,sd=1)
            }else{
        ...
        for ( i in 3:n ){
            eta.1 <- eta.2
            if(check.innovation & P[i]<=d & P[i-1]<=d & P[i-2]<=d){
                if(Q.obs[i]>0){
                    ##integr <- min(integrate(f=f.inno, lower=lower, upper=Q.obs[i], Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], scale.prev=scale[i-1], sc=scale[i], de=(t.obs[i]-t.obs[i-1])/tau, dff=df, gmm=gamma, subdivisions=2000)$value,1)
                    ##res.stdnorm[i] <- qnorm(integr)
                    delta <- (t.obs[i]-t.obs[i-1])/tau
    ...
    }else if(grepl("LogLikelihoodHydrology_la", su$settings$f.likeli)){
        if ( Q.obs[1] > 0 ){
            res.stdnorm[1] <- qnorm(prob.t((Q.obs[1]-Q.mod[1])/scale[1],df=df,gamma=gamma))
        }else{
            rndun <- runif(1)
            cumuzero <- prob.t(-1*Q.mod[1]/scale[1],df=df,gamma=gamma)
            res.stdnorm[1] <- qnorm(p=cumuzero*rndun)
        }
        for ( i in 2:n ){
            if(check.innovation){
                if(Q.obs[i]>0){
                    res.stdnorm[i] <- qnorm(min(integrate(f=f.inno, lower=lower, upper=Q.obs[i], Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], scale.prev=scale[i-1], sc=scale[i], de=(t.obs[i]-t.obs[i-1])/tau, dff=df, gmm=gamma)$value,1))
                }else{
                    rndun <- runif(1)
                    cumuzero <- min(integrate(f=f.inno, lower=lower, upper=0, Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], scale.prev=scale[i-1], sc=scale[i], de=(t.obs[i]-t.obs[i-1])/tau, dff=df, gmm=gamma)$value,1)
                    res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
                }
            }else{
                if ( Q.obs[i] > 0 ){
    ...
    if(grepl("LogLikelihoodHydrology_mod_n", su$settings$f.likeli)){
        if ( Q.obs[1] > 0 ){
            res.stdnorm[1] <- qnorm(pnorm(Q.obs[1], mean=Q.mod[1], sd=sd[1]))
        }else{
            rndun <- runif(1)
            cumuzero <- pnorm(0, mean=Q.mod[1], sd=sd[1])
            res.stdnorm[1] <- qnorm(p=cumuzero*rndun)
        }
        for ( i in 2:n ){
            if(check.innovation & (Q.mod[i] < Q.mod[i-1]) & (Q.obs[i-1] > 0)){
                if(Q.obs[i]>0){
                    res.stdnorm[i] <- qnorm(min(integrate(f=f.inno.n, lower=lower, upper=Q.obs[i], Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], sd.prev=sd[i-1], sdd=sd[i], de=(t.obs[i]-t.obs[i-1])/tau)$value,1))
                }else{
                    rndun <- runif(1)
                    cumuzero <- min(integrate(f=f.inno.n, lower=lower, upper=0, Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], sd.prev=sd[i-1], sdd=sd[i], de=(t.obs[i]-t.obs[i-1])/tau)$value,1)
                    res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
                }
            }else{
                if ( Q.obs[i] > 0 ){
                    ...
                    rndun <- runif(1)
                    cumuzero <- pnorm(0, mean=Q.mod[i], sd=sd[i])
                    res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
                }
            }
        }
    }else if(grepl("LogLikelihoodHydrology_mod", su$settings$f.likeli)){
        if ( Q.obs[1] > 0 ){
            res.stdnorm[1] <- qnorm(pt((Q.obs[1]-Q.mod[1])/scale[1],df=df))
        }else{
            rndun <- runif(1)
            cumuzero <- pt(-1*Q.mod[1]/scale[1],df=df)
            res.stdnorm[1] <- qnorm(p=cumuzero*rndun)
        }
        for ( i in 2:n ){
            if(check.innovation & (Q.mod[i] < Q.mod[i-1]) & (Q.obs[i-1] > 0)){
                if(Q.obs[i]>0){
                    res.stdnorm[i] <- qnorm(min(integrate(f=f.inno.t, lower=lower, upper=Q.obs[i], Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], scale.prev=scale[i-1], sc=scale[i], de=(t.obs[i]-t.obs[i-1])/tau, dff=df)$value,1))
                }else{
                    rndun <- runif(1)
                    cumuzero <- min(integrate(f=f.inno.t, lower=lower, upper=0, Q.obs.prev=Q.obs[i-1], Q.sim.prev=Q.mod[i-1], y=Q.mod[i], scale.prev=scale[i-1], sc=scale[i], de=(t.obs[i]-t.obs[i-1])/tau, dff=df)$value,1)
                    res.stdnorm[i] <- qnorm(p=cumuzero*rndun)
                }
            }else{
                if ( Q.obs[i] > 0 ){
    ...
    res.stdnorm <- pmax(res.stdnorm, -10)
    res.stdnorm <- pmin(res.stdnorm, 10)
    return(res.stdnorm)
}
