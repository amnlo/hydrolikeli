...
## if ( ! require(utils) )     { install.packages("rjson");     library(rjson) }
...
plot.states 		<- TRUE
set.par             <- NA#"../../A1Str07h2x/v05_connred2aggr_QE3P7c07i_TTE1b02c08i_SE1c08_priorExb0/opt_par_v05_connred2aggr_QE3P7c07i_TTE1b02c08i_SE1c08_priorExb0.txt"
plot.uncert		 	<- FALSE
...
start.par          <- paste0("../../A1",struct,"/v05_connrd1aggr_QE3P7c07i_TT",errmod.conc,"b02c08i_SE1c08_prior",pritag,"/opt_par_v05_connrd1aggr_QE3P7c07i_TT",errmod.conc,"b02c08i_SE1c08_prior",pritag,".txt")
vary			   <- 1.2 # vary the initial parameters by this factor to create the starting interval
time.units <- "hours"
...
close(file.config)
su <- model.setup(su, settings=sett, writeO=TRUE, f.path.hru=f.path.hru, f.path.transK=NA, f.path.hyperfun=ifelse(grepl("SE[0-9]",tag), f.path.hyperfun, NA)) ## ATTENTION: HRU file is read here! It has to agree with the object that is loaded. (f.path.config).
if(!reload){
	...
	if(is.null(su$optimized)){
		init.range <- redef.init.range(su)
...
if(run.timedep){
	su$model$parameters["Glo%Cmlt_K_Qq_SR"] <- -9
	su$model$parameters["Glo%Cmlt_AlQq_SR"] <- 1
	res.timedep <- run.model(layout = su$layout, sudriv=su)$incld.lmpd
	save(res.timedep, file=paste(savepath, "/restimedep1_", tag, ".RData", sep=""), version=2)
	su$model$parameters["Glo%Cmlt_K_Qq_SR"] <- -5
	su$model$parameters["Glo%Cmlt_AlQq_SR"] <- 0.1
	res.timedep <- run.model(layout = su$layout, sudriv=su)$incld.lmpd
	save(res.timedep, file=paste(savepath, "/restimedep2_", tag, ".RData", sep=""), version=2)

	# td <- pmin(pmax(randou(mean=0.95,sd=0.01,tau=10000,t=1:nrow(su$input$inputobs))$y, 0.51), 0.999)
	# save(td, file=paste(savepath, "/td1_", tag, ".RData", sep=""))
	# su$model$timedep$par <- matrix(td,ncol=1)
	# su$model$timedep$pTimedep <- names(su$model$parameters) %in% c("Glo%Cmlt_Dspl_SD")
	# res.timedep <- run.model(layout = su$layout, sudriv=su)$incld.lmpd
	# save(res.timedep, file=paste(savepath, "/restimedep1_", tag, ".RData", sep=""))

	# td <- pmin(pmax(randou(mean=0.7,sd=0.01,tau=10000,t=1:nrow(su$input$inputobs))$y, 0.51), 0.999)
	# save(td, file=paste(savepath, "/td2_", tag, ".RData", sep=""))
	# su$model$timedep$par <- matrix(td,ncol=1)
	# res.timedep <- run.model(layout = su$layout, sudriv=su)$incld.lmpd
	# save(res.timedep, file=paste(savepath, "/restimedep2_", tag, ".RData", sep=""))
...
if(plot.chains){
    ...
    plot.markov.hist(sudriv=su, brn.in=brn.in, pridef=pridef,lower.logpost=lower.logpost, plot=TRUE, file.hist=paste(savepath, "/hist", tag, ".pdf", sep=""), width=width, height=height, prior.only=FALSE, kl.div=FALSE, lab=modNam)
...
if(pred){
    su <- select.maxlikpars(su)
    m <- which(su$posterior.sample == max(su$posterior.sample))[1]
...
if(plot.uncert){
	...
	pdf(paste(savepath, "/predictions/", tag, "_E1E2E3E4.pdf", sep=""), width=10, height=7)
...
if(!is.na(compare.models[1])){
	...
	for(mod.curr in names(compare.models)){
		...
		savepath <- paste0("../output/", su$settings$subcatchment, su$settings$structure, "/", tag.mult[mod.curr])
	...
	if(export){
		...
		print("tmp: ")
		print(tmp)
		fil <- paste0("../output/modelcomparison/",paste0(names(compare.models),collapse=""),"stoch_E1E2E3E4.pdf")
	...
	pdf(paste0("../output/modelcomparison/",paste0(names(compare.models),collapse=""),"_E0.pdf"))
	plot.predictions(list.su=list.su, xlim=list(E1=E0), tme.orig=tme.orig,arrange=arrange,plot.var=plot.var)
	dev.off()
	pdf(paste0("../output/modelcomparison/",paste0(names(compare.models),collapse=""),"_E0zoom.pdf"))
	plot.predictions(list.su=list.su, xlim=list(E1=E0zoom), tme.orig=tme.orig,arrange=arrange,plot.var=plot.var)
	dev.off()
	#png(paste0("../output/modelcomparison/",paste0(names(compare.models),collapse=""),"_E0.png"), height=6, width=6, units="in", res=300)
	#plot.predictions(list.su=list.su, xlim=list(E1=E0), tme.orig=tme.orig,arrange=arrange,plot.var=plot.var)
	#dev.off()
	pdf(paste0("../output/modelcomparison/",paste0(names(compare.models),collapse=""),"_E1E2E3E4.pdf"), width=10, height=7)
